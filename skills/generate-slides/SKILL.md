...
- **New semantic icons:** the icon set in `scripts/lib/visgen/icons.py` is a closed,
  ...
  `references/icon-libraries/{simple-icons,tabler-icons,font-awesome,flaticon-uicons}`
  for a matching SVG (prefer the monoline outline weights; Font Awesome and UIcons
  are CC BY 4.0, so a vendored glyph carries an attribution obligation), vendor
